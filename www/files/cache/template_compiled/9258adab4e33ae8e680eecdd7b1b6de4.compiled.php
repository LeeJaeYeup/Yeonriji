...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_2','_notice.html');
} ?>
...
if($__Context->notice_list&&count($__Context->notice_list))foreach($__Context->notice_list as $__Context->no=>$__Context->document){ ?><li class="notice">
	<div class="tmb_wrp ribbon_v<?php echo $__Context->mi->ribbon_style ?>">
		...
		<img class="ie8_only shadow" src="/modules/board/skins/sketchbook5_2/img/ie/shadow.png" alt="" />
		<?php } ?>
...
<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->no=>$__Context->document){ ?><li<?php if($__Context->document_srl==$__Context->document->document_srl){ ?> class="select"<?php } ?>>
	<div class="tmb_wrp ribbon_v<?php echo $__Context->mi->ribbon_style ?>">
		...
		<a href="http://yeonriji.kr/index.php?mid=month&listStyle=webzine">
		<?php }else{ ?>
		...
		<div>
			...
			<div class="custom_content">
				...
				<p class="plus">+</p>
				<?php }else{ ?>
				<?php if($__Context->mi->gallery_tl=='N'){ ?><p class="name_ko"><?php echo $__Context->document->getTitle($__Context->mi->gall_tl_cut) ?></p><?php } ?>
				<?php } ?>
			</div>
			<?php if($__Context->dx == 8){ ?>
			<div class="custom_img-coverplus"></div>
			<?php }else{ ?>
