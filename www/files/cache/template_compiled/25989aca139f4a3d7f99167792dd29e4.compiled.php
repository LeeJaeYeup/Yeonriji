...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_1','_notice.html');
} ?>
...
if($__Context->notice_list&&count($__Context->notice_list))foreach($__Context->notice_list as $__Context->no=>$__Context->document){ ?><li class="notice">
	<div class="tmb_wrp ribbon_v<?php echo $__Context->mi->ribbon_style ?>">
		...
		<img class="ie8_only shadow" src="/modules/board/skins/sketchbook5_1/img/ie/shadow.png" alt="" />
		<?php } ?>
...
<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->no=>$__Context->document){ ?><li<?php if($__Context->document_srl==$__Context->document->document_srl){ ?> class="select"<?php } ?>>
	<div class="tmb_wrp ribbon_v<?php echo $__Context->mi->ribbon_style ?>">
		...
		<?php if(!$__Context->document->thumbnailExists()){ ?><span class="no_img tmb">No Image</span><?php } ?>
		<?php if($__Context->document->thumbnailExists()){ ?>
		<?php if($__Context->mi->tmb_effect=='2'){ ?><span class="no_img loading"></span><?php } ?>
		
		
		<a <?php if(!$__Context->mi->link_board){ ?>href="<?php echo getUrl('document_srl',$__Context->document->document_srl,'listStyle',$__Context->listStyle,'cpage','') ?>"
		<?php }else{ ?>href="<?php echo $__Context->document->getExtraEidValue('link_url') ?>" target="_self"
		<?php } ?>data-viewer="<?php echo getUrl('document_srl',$__Context->document->document_srl,'listStyle','viewer','page','') ?>">
		<div>
			<img class="tmb" src="<?php echo $__Context->document->getThumbnail($__Context->mi->thumbnail_width, $__Context->mi->thumbnail_height, $__Context->mi->thumbnail_type) ?>" alt="">
			<div class="custom_content">
				<?php if($__Context->mi->gallery_tl=='N'){ ?><p class="name_ko"><?php echo $__Context->document->getExtraEidValue('name_kr') ?></p><?php } ?>
				
				<?php if($__Context->mi->gallery_tl=='N'){ ?><p class="name_us"><?php echo $__Context->document->getExtraEidValue('name_us') ?></p><?php } ?>
			</div>
			<div class="custom_img-cover"></div>
		</div>
		</a>
...
echo $__Context->mi->tmb_hover;
					...
					<span class="padding">
						<?php if($__Context->mi->gallery_tl=='N'){ ?><b class="tl"><?php echo $__Context->document->getTitle($__Context->mi->gall_tl_cut) ?> <?php echo $__Context->dx ?></b><?php } ?>
						<!-- <?php if($__Context->list_config['regdate']){ ?><b class="tl"><?php echo $__Context->document->getRegdate("Y/m/d") ?></b><?php } ?>
						<?php if($__Context->list_config['nick_name']){ ?>by <b><?php echo $__Context->document->getNickName() ?> <?php echo $__Context->dx ?></b><br /><?php } ?>
						<?php if($__Context->mi->use_category=='Y' && $__Context->document->get('category_srl')){ ?>in <b><?php echo $__Context->category_list[$__Context->document->get('category_srl')]->title ?></b><br /><?php } ?>
						<span><?php if($__Context->list_config['readed_count']){ ?>Views <b><?php echo $__Context->document->get('readed_count') ?></b>&nbsp;<?php } ?> <?php if($__Context->list_config['voted_count']){ ?>Likes <b><?php echo $__Context->document->get('voted_count') ?></b><?php } ?></span>
						<?php if($__Context->document->getCommentCount()){ ?>Replies <b><?php echo $__Context->document->getCommentCount() ?></b><?php } ?> -->
					</span>
