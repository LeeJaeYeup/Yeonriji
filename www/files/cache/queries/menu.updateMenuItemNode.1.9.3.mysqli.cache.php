...
${'menu_srl6_argument'} = new Argument('menu_srl', $args->{'menu_srl'});
if(!${'menu_srl6_argument'}->isValid()) return ${'menu_srl6_argument'}->getErrorMessage();
} else
${'menu_srl6_argument'} = NULL;if(${'menu_srl6_argument'} !== null) ${'menu_srl6_argument'}->setColumnType('number');
...
${'parent_srl7_argument'} = NULL;if(${'parent_srl7_argument'} !== null) ${'parent_srl7_argument'}->setColumnType('number');
if(isset($args->listorder)) {
${'listorder8_argument'} = new Argument('listorder', $args->{'listorder'});
if(!${'listorder8_argument'}->isValid()) return ${'listorder8_argument'}->getErrorMessage();
} else
${'listorder8_argument'} = NULL;if(${'listorder8_argument'} !== null) ${'listorder8_argument'}->setColumnType('number');
...
new UpdateExpression('`menu_srl`', ${'menu_srl6_argument'})
,new UpdateExpression('`parent_srl`', ${'parent_srl7_argument'})
,new UpdateExpression('`listorder`', ${'listorder8_argument'})
));
