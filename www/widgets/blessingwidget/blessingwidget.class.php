 ...
 * @version 0.1
 */
class blessingwidget extends WidgetHandler
{
	/**
	...
	function proc($args)
	{
		...
		else
		{
			...
			if(!$args->module_srls)
			{
				...
				$output = executeQueryArray('widgets.blessingwidget.getMids', $obj);
				if($output->data)
				{
	...
	function _getDocumentItems($args)
	{
		...
		$obj = new stdClass();
		$obj->module_srl = $args->module_srl;
		$output = executeQueryArray('widgets.blessingwidget.getCategories',$obj);
		...
		$output = executeQueryArray('widgets.blessingwidget.getNewestDocuments', $obj);
	...
	function _compile($args,$content_items)
	{
		...
		return $oTemplate->compile($tpl_path, "blessingwidget");
	}
}
